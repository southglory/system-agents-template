...
## Multi-Agenten-Kompatibilität (Claude & Antigravity)

Diese Vorlage ist so konzipiert, dass sie sowohl **Claude Code**- als auch **Antigravity**-Agenten (Google) gleichzeitig unterstützt.

- **Claude Code**: Verwendet primär `.system-agents/` für das rundenbasierte Zustandsmanagement und `/skills/` für CLI-Befehle.
- **Antigravity**: Nutzt `.agents/workflows/` für die **Turbo-automatisierte** Aufgabenausführung.
- **Zusammenarbeit**: Beide Agenten teilen sich `board.yaml` (Aufgaben) und `chatrooms/` (Verlauf). Antigravity kann das rundenbasierte Protokoll einhalten, indem es vor Arbeitsbeginn die `board.yaml` prüft.
